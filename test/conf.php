...
function filterXSS($val) {
  ...
  /*  Finaliza Agregado */
	...
	$dbserver 	= "localhost";
	$database 	= "simple_almacen";
	$user 		= "simple";
	$pswd 		= "12345";	
	
	
	
	//$vinculo  	= @mysql_connect($dbserver,$user,$pswd);
	$vinculo    = @mysqli_connect($dbserver, $user, $pswd, $database);
	//if(mysql_error()) {
	if(mysqli_connect_errno($vinculo)){
		//header ("Location: /rh/edesempeno/error_pages/error_conec.php");
		echo "error1 ". mysqli_connect_error();
		exit;	
	}
/*
	$conexion 	= mysql_select_db($database,$vinculo);
...
if (!mysqli_set_charset($vinculo, "utf8")) {
    echo "Error loading character set utf8: ".mysqli_error($vinculo);
	exit;
} 
