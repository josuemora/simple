...
function simplePlantilla1($aDat=array()){
	...
	$pDlgAccionAlLlenarReg = isset($aDat['AccionAlLlenarReg']) ? $aDat['AccionAlLlenarReg'] : '';
		...
		foreach($renglon['columnas'] as $columna){
			...
			foreach($columna['objetos'] as $NumRen=>$objeto){
				...
				if($objeto['tipo']=='etiqueta' || $objeto['tipo']=='html'){
					...
					if(strpos($objeto['referencia'],"type=\"checkbox\"")!==false 
						|| strpos($objeto['referencia'],"type='checkbox'")!==false 
						|| strpos($objeto['referencia'],"type=\"radio\"")!==false 
						|| strpos($objeto['referencia'],"type='radio'")!==false){
						$flagCheckboxradio = 1;
					}
				}
